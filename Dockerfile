...
ADD https://api.github.com/repos/earsnot/picoGW_hal/git/refs/heads/master version.json
RUN git clone -b master https://github.com/earsnot/picoGW_hal.git && \
    cd /picoGW_hal && make clean all

# ADD the GitHub API's representation of repo to a dummy location. 
# The API call will return different results when the head changes, invalidating the docker cache.
ADD https://api.github.com/repos/earsnot/picoGW_packet_forwarder/git/refs/heads/master version.json
RUN git clone -b master https://github.com/earsnot/picoGW_packet_forwarder.git && \
